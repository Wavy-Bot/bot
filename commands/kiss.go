...
func KissHandler(e *handler.CommandEvent) error {
	...
	case discord.ApplicationCommandTypeSlash:
		...
		switch ok {
		case true:
			message = fmt.Sprintf("%s kisses %s", e.Member().EffectiveName(), member.EffectiveName())
		case false:
			member = *e.Member()
			message = fmt.Sprintf("%s kisses themselves", member.EffectiveName())
		}
	case discord.ApplicationCommandTypeUser:
		...
		case true:
			message = fmt.Sprintf("%s kisses themselves", executor.EffectiveName())
		case false:
			message = fmt.Sprintf("%s kisses %s", executor.EffectiveName(), member.EffectiveName())
		}
	}
